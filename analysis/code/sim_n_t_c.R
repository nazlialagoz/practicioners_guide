...
sim_data <- function(num_unit, num_period, cohort_periods, tau = 1.00){
  constant = 80
  data <- as.data.table(init_data(num_unit = num_unit, 
                                  num_period = num_period, 
                                  tau = tau, 
                                  cohort_periods = cohort_periods, 
                                  constant = constant))
  
  # Set names and keys as needed
  # setnames(data, 'dep_var', 'your_variable_name')
  setkeyv(data, c('cohort_period', 'unit', 'period'))
  setkeyv(data, c('unit', 'period'))
